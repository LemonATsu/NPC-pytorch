...
def read_spin_data(
    data_path: str, 
    ext_scale: float = 0.001, 
    img_res: int = 1000, 
    bbox_res: int = 224
):
    """ Obselete ... no longer use SPIN anymore
    """
...
def convert_crop_cam_to_orig_img_and_focal(
    cam: np.ndarray, 
    bbox: np.ndarray, 
    img_width: int, 
    img_height: int,
    focal: float = 5000., 
    resized_width: int = 224, 
    resized_height: int = 224,
    new_focal: Optional[float] = None,
):
    """
    Borrow from VIBE
    """
...
def get_keypoints_from_betas(
    betas: torch.Tensor, 
    joints: torch.Tensor, 
    rot_mats: torch.Tensor,
    ext_scale: float = 1.0, 
    align_joint_idx: int = 8,
    ref_pose: np.ndarray = smpl_rest_pose,
    scale_rest_pose: bool = True,
    mapper: List[int] = SMPL_JOINT_MAPPER, 
    gender: str = 'NEUTRAL'
):
    '''
...
def pred_cams_to_orig_cam_params(
    cameras: np.ndarray, 
    bboxes: np.ndarray,
    img_width: int = 512, 
    img_height: int = 512,
    ...
    orig_cams = convert_crop_cam_to_orig_img_and_focal(
        cameras, 
        bboxes,
        img_width=img_width,
...
def process_spin_data(
    ...
    resized_res: int = 224, 
    ext_scale: float = 0.001,
    dataset_ext_scale: float = 0.25 / 0.00035,
    align_surreal_cam: bool = False,
    scale_rest_pose: bool = True,
    new_focal: Optional[float] = None,
    skel_type: Skeleton = SMPLSkeleton
):
    ...
    focals, c2ws = pred_cams_to_orig_cam_params(
        cameras, bboxes,
        img_width=res_W,
        img_height=res_H,
