...
try:
    ...
    print('old pytorch version: need newer torchvision for dataprocessing')
...
def process_aist_data(
    data_path, 
    subject='d01', 
    training_views=[0, 3, 6], 
    split='train', 
    fps=10, 
    ...
    for seq_name in seq_names:
        ...
        seq_kps, seq_bones, seq_skts, rest_pose = aist_dataset.load_smpl_data(
            motion_path, 
            seq_name, 
            apply_scale=False
        )
    ...
    bkgds = [
        cv2.undistort(imageio.imread(p), intrins[i], distorts[i] * 0.0) 
    ...
    for i, (video_name, view_id, intrin, distort) in enumerate(zip(video_names, view_ids, intrinsics, distortions)):
        ...
        seq_imgs = aist_dataset.load_frames(
            os.path.join(video_path, f'{video_name}.mp4'), 
            frame_ids=np.arange(1e6).tolist(), 
            fps=fps
        )
        seq_masks = aist_dataset.load_frames(
            os.path.join(mask_path, f'{video_name}_alpha2.mp4'), 
            frame_ids=np.arange(1e6).tolist(), 
            fps=fps
            )
