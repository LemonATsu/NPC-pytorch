...
def kp_to_valid_rays(poses, H, W, focal, kps=None, cylinder_params=None,
                     skts=None, centers=None, ext_scale=0.001):
...
def get_ray_box_intersections(rays_o, rays_d, bound_range=1., eps=1e-5):
    ...
    # -> move along ray direction by t will reach x_min

    B, J = rays_o.shape[:2]
    bounds = bound_range * torch.ones(1, J, 2, 3)#.double()
    bounds[..., 0, :] *= -1

    # the following case seems to happen when the ray somehwhat close to (0, 0, 0)
    # this happens a lot more often in orthographic projection..
    # extremely tiny positive values
    # make the "small ray dir value" slightly larger than the volume bound eps
    # we don't need the casting to be super accurate anyway
    rays_d[(rays_d < eps) & (rays_d > -1e-10)] = eps * 1.
    # extremely tiny negative values
    rays_d[(rays_d > -eps) & (rays_d < 1e-10)] = -eps * 1.

    # we need double in this function for precision ..
    ...
    # Also, back to float as we don't need double precision anymore
    ...
    # find the 1. valid volume, and 2. get the two points from the valid volume 
    p_intervals = p_intersect[v_valid][p_valid[v_valid]]
    p_intervals = p_intervals.reshape(-1, 2, 3)

    return p_valid, v_valid, p_intervals

def conical_frustum_to_gaussian(rays_d, t0, t1, radii=1.0, diag=True):

    mp = (t0 + t1) / 2. # midpoint
    hw = (t1 - t0) / 2. # halfwidth

    mp_sq, hw_sq = mp.pow(2.), hw.pow(2.)
    hw_qd = hw_sq.pow(2.)
    # mean dist along ray
    mu_t = mp + 2 * mp * hw_sq / (3. * mp_sq + hw_sq)

    # var. along ray
    var_t = hw_sq / 3. - (4. / 15.) * (
                (hw_qd * 12. * mp_sq - hw_sq) / (3. * mp_sq + hw_sq)**2
            )
    # var. along the disc
    var_r = radii**2 * (
                mp_sq / 4. + 5. * hw_sq / 12. - (4. / 15.) * (
                    (hw_qd) / (3 * mp_sq + hw_sq)
                )
            )
    
    return lift_to_gaussian(rays_d, mu_t, var_t, var_r, diag=diag)

def lift_to_gaussian(rays_d, mu_t, var_t, var_r, diag=True):
    N_rays, N_samples = mu_t.shape
    # expand to (N_rays, 1, 3) and (N_rays, N_samples, 1)
    mu = rays_d[..., None, :] * mu_t[..., None]
    dnorm_sq = rays_d.pow(2.).sum(-1).reshape(N_rays, 1, 1).clamp(min=1e-8)

    if diag:
        # Eq 16. in MipNeRF
        dd = rays_d.pow(2.).reshape(N_rays, 1, 3)
        # covariance along t (ray) and r (disc)
        cov_t = var_t.reshape(N_rays, N_samples, 1) * dd
        cov_r = var_r.reshape(N_rays, N_samples, 1) * (
                1 - dd / dnorm_sq
        )
    else:
        # Eq 8. in MipNeRF
        ddT = rays_d.reshape(N_rays, 3, 1) * rays_d.reshape(N_rays, 1, 3)
        I = torch.eye(3).reshape(1, 3, 3).expand(N_rays, -1, -1)

        # covariance along t (ray) and r (disc)
        cov_t = var_t.reshape(N_rays, N_samples, 1, 1) * ddT.reshape(N_rays, 1, 3, 3)
        cov_r = var_r.reshape(N_rays, N_samples, 1, 1) * (
                        I - ddT / dnorm_sq
                ).reshape(N_rays, -1, 3, 3)
...
def get_dist_pts_to_lineseg(pts, p0, p1):
    ...
    proj_pts = p0 + proj[..., None] * seg_norm
    if torch.isnan(proj_pts).any():
        import pdb; pdb.set_trace()
        print
