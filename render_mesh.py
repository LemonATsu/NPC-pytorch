...
parser.add_argument('--basedir', type=str, default='render_output/')
#parser.add_argument('--dataset', type=str)
parser.add_argument('--mesh_ind', type=int, default=None)
parser.add_argument('--skip', type=int, default=1)
parser.add_argument('--flip', action='store_true',
                     help='flip the rendered geometry left to right')

args = parser.parse_args()

print('start creating renderer')
renderer = ColorRender(width=args.width, height=args.height)
print('done')

#data_root = 'data/result/if_nerf/{}/mesh'.format(
#    args.exp_name)
data_root = args.basedir

mesh_base_dir = os.path.join(data_root, 'mesh')

os.system('mkdir -p {}'.format(mesh_base_dir))
obj_files = sorted(glob.glob(os.path.join(data_root, 'mesh', '*.ply')))

if args.mesh_ind is None:
    obj_files = obj_files[::args.skip]
else:
    obj_files = obj_files[args.mesh_ind:args.mesh_ind+1]

R = make_rotate(math.radians(270), math.radians(180), math.radians(90))  
print(colored('the results are saved at {}'.format(mesh_base_dir), 'yellow'))
...
for i, obj_path in enumerate(obj_files):
    ...
    cam_rot = make_rotate(math.radians(5.), 0, 0)
    vertices = np.matmul(vertices, cam_rot.T)
    cnt = 0

    mesh_render_dir = os.path.join(mesh_base_dir, f'{i:03d}')
    ...
    for j in range(0, 361, 4):
        ...
        cv2.imwrite(os.path.join(mesh_render_dir, f'{cnt:03d}.jpg'), 255 * img)
        cnt += 1
